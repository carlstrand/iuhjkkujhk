            ...
            $(function(){
                ...
                $slider.slider({
                    ...
                    create:function(event, ui){
                        $('#'+ elementID + '_min').html(self.options.currency_symbol + $("#" + elementID + "_slider").slider('values', 0)).position({
                            my: 'center top',
                            at: 'center bottom',
                            of: $("#" + elementID + "_slider a:eq(0)")
                        });

                        $('#'+ elementID + '_max').html(self.options.currency_symbol + $("#" + elementID + "_slider").slider('values', 1)).position({
                            my: 'center top',
                            at: 'center bottom',
                            of: $("#" + elementID + "_slider a:eq(1)")
                        });
                    },
                    slide: function( event, ui ) {
                        var delay = function() {
                            var handleIndex = $(ui.handle).index();
                            var label = handleIndex == 1 ? '#'+ elementID + '_min' : '#'+ elementID + '_max';
                            $(label).html(self.options.currency_symbol + ui.value).position({
                                my: 'center top',
                                at: 'center bottom',
                                of: ui.handle
                            });
